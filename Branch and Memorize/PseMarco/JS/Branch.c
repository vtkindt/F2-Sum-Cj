...
int Istrcpy(short *Dest, short *Source);
...
/*						1 (Eliminabile)																				*/
/********************************************************************/
int FSDominance4withDB(Node *N, int i)
{
  
	int jp,j,k,t, n, *index2;
	int CSUMOld, CSUMNew, C2Old, C2New;
	short S[MAXJOBS], SOld[MAXJOBS], A, B, C, D, E;
...
int FSDB(Node *N, int *index2)
{
	...
	// DB Search
	// We first test with the active nodes
	*index2=0;
