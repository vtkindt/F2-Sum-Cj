...
short BestSol1[MAXJOBS];
short BestSol2[MAXJOBS];
int JH_C2;
short JHSequence[MAXJOBS];
...
int LoadDAT(char *DATName)
{
  ...
  while (fgets(s,20,Fp) != NULL) {
    ...
    sscanf(s,"%d %d %d %d",&JOB[Jobs].ID, &JOB[Jobs].p1, &JOB[Jobs].p2, &JOB[Jobs].Dir);
...
Node *GetNode(void)
{
 ...
 // We now check the dominance condition with the database
    ...
    if ((GetActualDBDimension()!=0)&&(FSDB(N,&index2)))
	{
